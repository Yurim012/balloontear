...
<% }else {
   ...
   category = membersignupDAO.categorySeach(id);
	...
	// 새 댓글 
	int freeReNo = 0, freeReplyGrp = 1, freeReplyStep = 0, freeReplyLevel = 0; 
	
	// 댓글의 댓글 
	if(request.getParameter("freeReNo") != null) {
		freeReNo = Integer.parseInt(request.getParameter("freeReNo"));
		freeReplyGrp = Integer.parseInt(request.getParameter("freeReplyGrp"));
		freeReplyStep = Integer.parseInt(request.getParameter("freeReplyStep"));
		freeReplyLevel = Integer.parseInt(request.getParameter("freeReplyLevel"));
	}
...
<%-- ******* 댓글작성 ****** --%>
	...
	<div class="container">
		<h4 align="center"><b>댓글 작성</b></h4>
	   	<div align="center">
		<form action="freeReplyPro.jsp?pageNum=<%=pageNum%>" method="post">
			<input type="hidden" name="boNo" value="<%=boNo%>"/>
			<input type="hidden" name="freeReNo" value="<%=freeReNo%>" />
			<input type="hidden" name="freeReplyGrp" value="<%=freeReplyGrp%>"/>
			<input type="hidden" name="freeReplyStep" value="<%=freeReplyStep%>"/>
			<input type="hidden" name="freeReplyLevel" value="<%=freeReplyLevel%>"/>
		<br />
	   <hr />
			<table>
				<tr>
					<td>내 용</td>
					<td><textarea rows="4" cols="100" name="freeReply"></textarea>  </td>
				</tr>
				<tr>
					<td>작성자</td>
					<%if(category.equals("cen")){ %>
						<td align="left"><input type="hidden" name="freeReplyer" value="<%=member.getMemName()%>"/><%=member.getMemName() %></td>
					<%}else{ %>
						<td align="left"><input type="hidden" name="freeReplyer" value="<%=id%>"/><%=id %></td>
					<%} %>
				</tr>
				<tr><td><br /></td></tr>
			</table>
						<input type="submit" value="댓글저장" class="btn btn-outline-secondary btn-sm"/> 
						<input type="button" value="취소" onclick="window.location='freeContent.jsp?boNo=<%=boNo%>&pageNum=<%=pageNum%>'" class="btn btn-outline-secondary btn-sm"/>
		</form>
			</div>
		<%} %>
	</div>
