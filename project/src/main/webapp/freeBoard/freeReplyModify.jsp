...
<%
   ...
   <% }else {
	...
	FreeReplyBoardDTO freeReply = dao.getOneFreeReply(freeReNo); 
...
<%-- ******* 댓글수정 ****** --%> 
	<div class="container">
		<h4 align="center"><b>댓글 수정</b></h4>
		<br />
	    <hr />
	    <div align="center">
			<form action="freeReplyModifyPro.jsp?pageNum=<%=pageNum%>" method="post">
				<input type="hidden" name="boNo" value="<%=boNo%>"/>
				<input type="hidden" name="freeReNo" value="<%=freeReNo%>"/>
				<table>
					<tr>
						<td>내 용</td>
						<td><textarea rows="4" cols="100" name="freeReply"><%=freeReply.getFreeReply() %></textarea>  </td>
					</tr>
					<tr>
						<td>작성자</td>
						<td align="left"><%=freeReply.getFreeReplyer()%></td>
					</tr>
				</table>
							<input type="submit" value="댓글수정" /> 
							<input type="button" value="취소" onclick="window.location='freeContent.jsp?boNo=<%=boNo%>&pageNum=<%=pageNum%>'" />
			</form>
			</div>
		</div>
	<%} %>
