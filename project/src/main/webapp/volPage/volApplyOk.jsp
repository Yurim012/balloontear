...
<head>
   ...
   <%
     ...
     category = dao.categorySeach(id);
      ...
      //volNo주고 봉사 모든 봉사 정보 가져오기 
      VolBoardDAO voldao = new VolBoardDAO();
      VolBoardDTO volArticle = voldao.getOneVolContent(volNo);
      
      //volNo주고 신청자 정보 가져오기
      VolApplyBoardDAO volAdao = new VolApplyBoardDAO();
      VolApplyBoardDTO volApplyArticle = volAdao.getOneApplyerInForm(volNo);  
      
      //id 주고 이름 받기
      MemberSignupDAO memdao = new MemberSignupDAO(); 
      String userName = memdao.getName(id);
      String phone = memdao.getPhone(id);
      String email = memdao.getEmail(id);
   %> 
...
<div class="container">
	...
	<div align="center">
		<div class="card shadow-sm" id="volApply" style="background-color:#f8fafc; width: 800px; padding: 40px;">
			<section>
				<br />
				<h1 align="center">봉사 신청이 완료되었습니다 🎈 </h1>
				<br />
				<br />
			<table style="font-size: 20px; margin: 30px;">
				...
				<br />
				<br />
			<input type="button" value="목록으로" onclick="window.location='volBoardMain.jsp?volNo=<%=volNo%>&pageNum<%=pageNum %>'" class="btn btn-outline-secondary btn-sm"/>
			<input type="button" value="메인으로" onclick="window.location='/project/main/memberMain.jsp'" class="btn btn-outline-secondary btn-sm"/>
