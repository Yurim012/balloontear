...
<%
	   ...
	   // 전체 글의 개수 구하고, 
	   // DB에서 전체 글 개수 가져오기
	   int volEndCount = 0;             // (전체/검색) 전체 글 개수 담을 변수 
	   List<VolBoardDTO> volEndList = null;;    // (전체/검색) 글 목록 리턴받을 변수 
   ...
   // 검색 여부 판단 +//위 검색 여부 
      ...
      if(sel != null && search != null) { // 검색일때 
    	  volEndCount = volboarddao.getVolSearchEndCount(sel, search);  // 검색에 맞는 게시글에 개수 가져오기 
         if(volEndCount > 0) {  
            // 검색한 글 목록 가져오기 
            volEndList = volboarddao.getVolEndSearch(startRow, endRow, sel, search);  
         }
      }else { // 일반 게시판일때  
    	  volEndCount = volboarddao.getVolEndCountWithCen(userName);  // 그냥 전체 게시글 개수 가져오기 
         // 글이 하나라도 있으면, 현재 페이지에 띄워줄 만큼만 가져오기 (페이지 번호 고려) 
         if(volEndCount > 0){
        	 volEndList =  volboarddao.getVolEndWithCen(startRow, endRow, userName); 
         }  
      } 
      System.out.println("article count : " + volEndCount);
      System.out.println(volEndList);
      
      int number = volEndCount - (currentPage - 1) * pageSize; // 화면에 뿌려줄 글번호(bno아님)
...
<br />
 ...
 <% } %>
	...
	<div class="container">
		<div class="row">
			<div class="col-sm">
					<h4 align="center"><b>모집마감 공고 전체목록</b></h4>
                    ...
                    <%-- 반복해서 뿌리기  --%>
                     ...
                     <%for(int i = 0; i < volEndList.size(); i++){ 
                           VolBoardDTO volArticle= volEndList.get(i); 
                        ...
                        <tr>
                           ...
                           <td class="tableTd">
                          	 <button class="btn btn-outline-secondary btn-sm" onclick="window.location='volEndApplyList.jsp?volNo=<%=volArticle.getVolNo()%>'">지원자 보기</button>
                           <td>
                        </tr>
						<%} %>
                        <%} %>
	...
	<br />
 	...
 	<div class="row">
   	...
   	<% if(volEndCount > 0) { 
         ...
         int pageNumSize = 5; 
         // 총 몇페이지 나오는지 계산 
         int pageCount = volEndCount / pageSize + (volEndCount % pageSize == 0 ? 0 : 1);
      ...
      for(int i = startPage; i <= endPage; i++) { 
         if(sel != null && search != null) { %>
            <a class="Atitle" href="volEndRecruitList.jsp?pageNum=<%=i%>&sel=<%=sel%>&search=<%=search%>"> &nbsp; <%= i %> &nbsp; </a>
         <%}else{ %>
            <a class="Atitle" href="volEndRecruitList.jsp?pageNum=<%=i%>"> &nbsp; <%= i %> &nbsp; </a> 
         <%} 
      }
      
      if(endPage < pageCount) { 
         if(sel != null && search != null) { %>
            <a class="Atitle" href="volEndRecruitList.jsp?pageNum=<%=startPage+pageNumSize%>&sel=<%=sel%>&search=<%=search%>"> &nbsp; &gt; </a>
      <%   }else{ %>
            <a class="Atitle" href="volEndRecruitList.jsp?pageNum=<%=startPage+pageNumSize%>"> &nbsp; &gt; </a>
      <%}
      } 
      } %>
      ...
      <br /><br />
       <%-- 작성자/내용 검색 --%>
      <form action="volEndRecruitList.jsp">
         <select name="sel">
      ...
      <button class="btn btn-outline-secondary btn-sm" onclick="window.location='volEndRecruitList.jsp'"> 전체 목록 </button>
  </div>
  </div>
  </div>
  </div>
...
<body>
                    ...
                    <%-- 반복해서 뿌리기  
                     ...
                     <%for(int i = 0; i < volList.size(); i++){ 
                        ...
                        <tr>
                           ...
                           <td>
                          	 <button onclick="window.location='volEndApplyList.jsp?volNo=<%=volArticle.getVolNo()%>'">지원자 보기</button>
                           <td>
                        </tr>
                        <%} %>
                        <%} %>
	...
	<br />
 	<%-- 게시판 목록 페이지 번호 뷰어 
      ...
      <button onclick="window.location='volEndRecruitList.jsp'"> 전체 목록 </button>
